...
const msOffset = new Date() - offsetFromDate;
const dayOffset = msOffset / 1000 / 60 / 60 / 24;
const targetWord =
  targetWords[Math.floor(dayOffset)];
...
function pressKey(key) {
  ...
  if (activeTiles.length >= worldLength) return;
...
function deleteKey() {
  ...
  const lastTile =
    activeTiles[activeTiles.length - 1];
...
function checkWinLose(guess, array) {
  ...
  const remainingTiles = guessGrid.querySelector(
    ":not([data-letter])"
  );
...
function danceTiles(tiles) {
  tiles.forEach((tile, index) => {
    ...
    }, (DANCE_ANIMATION_DURATION * index) / 5);
  });
}
